...
class WeldSeamsSortModel(BaseModel):
    welding_seam_sort: Annotated[
        list[WeldSeamSortModel],
        Field(
            description="多个焊缝，对焊缝的焊接顺序进行排序"
        )
    ]
...
class SolderJointMixedWeldSeamSortModel(BaseModel):
    """焊点集合混合焊缝的方案"""
    type_flag: Literal['SolderJointMixedWeldSeamSortModel'] = 'SolderJointMixedWeldSeamSortModel'
...
class WeldingSequenceSortModel(BaseModel):
    sequence_plan: Union[SolderJointsSortModel, SolderJointMixedWeldSeamSortModel] = Field(
